...
using SPSegment = simpledb::SPSegment;
using SchemaSegment = simpledb::SchemaSegment;
using SlottedPage = simpledb::SlottedPage;
using TID = simpledb::TID;

namespace schema = simpledb::schema;
...
TEST_F(SegmentTest, SPFuzzing) {
   ...
   for (size_t i = 0; i < lengths.size(); i++) {
      ...
      ASSERT_EQ(std::equal(buffer.begin(), buffer.end(), records[i].begin()), true)
         ...
         << "[ ERASE   ] -\n"
         << "EXPECTED:\n"
         << simpledb::hex_dump_str(reinterpret_cast<std::byte*>(records[i].data()), lengths[i])
         << "\nHAVE:\n"
         << simpledb::hex_dump_str(reinterpret_cast<std::byte*>(buffer.data()), lengths[i])
         << "\n";
   }
