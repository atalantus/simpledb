...
struct SlottedPage {
   struct Header {
      ...
      /// If greater or equal to slot_count than all slots are in use.
   ...
   Slot& get_slot(uint16_t slot_id);
   [[nodiscard]] const Slot& get_slot(uint16_t slot_id) const;
   ...
   [[nodiscard]] uint32_t get_fragmented_free_space() const;

   /// Allocate a slot.
   /// @param[in] data_size    The slot that should be allocated
   /// @param[in] page_size    The new size of a slot
   uint16_t allocate(uint32_t data_size, uint32_t page_size, bool is_redirect_target = false);

   /// Relocate a slot.
   /// TODO: Do we want to keep the same slot id?
